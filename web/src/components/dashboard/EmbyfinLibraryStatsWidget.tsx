...
import { useEmbyfinLibraryStats, type EmbyfinKind } from '@/lib/dashboardWidgets';
import { cn } from '@/lib/utils';
import type { ServiceInstance } from '@/lib/api';

const TYPE_ICON: Record<string, typeof Film> = { movies: Film, tvshows: Tv, music: Music };

export function EmbyfinLibraryStatsWidget({ instance, title, kind }: { instance: ServiceInstance; title: string; kind: EmbyfinKind }) {
  const navigate = useNavigate();
  const [refreshing, setRefreshing] = useState(false);
  const definition = getServiceDefinition(kind);
  const Icon = getServiceIcon(kind);
  const { stats, isLoading, error, refetch } = useEmbyfinLibraryStats(instance, kind);
