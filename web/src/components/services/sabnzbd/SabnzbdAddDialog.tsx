...
import { proxyApi, sabnzbdApi, type ServiceInstance } from '@/lib/api';

export function SabnzbdAddDialog({ instance, open, onOpenChange }: { instance: ServiceInstance; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const addUrl = useMutation({
    mutationFn: (nzbUrl: string) => proxyApi.call(instance.id, { path: '/api', query: { mode: 'addurl', name: nzbUrl } }),
    onSuccess: (res) => {
  ...
  const uploadFile = useMutation({
    mutationFn: (file: File) => sabnzbdApi.uploadNzb(instance.id, file),
    onSuccess: (res) => {
      if (!res.ok) return toast.error(res.error || 'Upload failed');
