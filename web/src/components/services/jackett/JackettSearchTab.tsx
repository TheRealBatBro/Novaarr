...
import { JACKETT_SEARCH_PATH } from './JackettApi';
...
export function JackettSearchTab({ instance }: { instance: ServiceInstance }) {
  const [query, setQuery] = useState('');
  const [results, setResults] = useState<TorznabItem[] | null>(null);

  const search = useMutation({
    mutationFn: (q: string) => proxyApi.call<TorznabResponse>(instance.id, { path: JACKETT_SEARCH_PATH, query: { t: 'search', q } }),
    onSuccess: (res) => {
      if (!res.ok) {
  ...
  return (
    <div>
      <form onSubmit={handleSubmit} className="mb-4 flex gap-2">
        <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search across all configured indexers…" className="flex-1" />
