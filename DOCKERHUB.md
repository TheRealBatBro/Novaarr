...
| `TAILSCALE_HOSTNAME` | unset | Cosmetic fallback link for the Tailscale status card if you're using the optional `tailscale` sidecar — the sidecar's own reported hostname takes priority when available. See the [full README](https://github.com/TheRealBatBro/Novaarr#tailscale). |
